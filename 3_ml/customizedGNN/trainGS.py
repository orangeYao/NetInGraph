...
def getTensor(adj_lists, feat_data, labels, edgesAttr):
    ...
    edgesAttr = torch.FloatTensor(edgesAttr.T)
    return edges.to(device), feat_data.to(device), labels.to(device), edgesAttr.to(device)


def run_cora_helper(feat_data, labels, adj_lists, edgesAttr, Net, 
                    feat_data_t, labels_t, adj_lists_t, edgesAttr_t,
                    design_names, optimizer_f, save_name):
    ...
    print ('len(adj_lists), len(adj_lists_t),', len(adj_lists), len(adj_lists_t))
    ...
    for epoch in range(250):
        ...
        for i in range(len(adj_lists)):
            edges, feat_data_c, labels_c, edgesAttr_c = getTensor(adj_lists[i], feat_data[i], labels[i], edgesAttr[i] )
            pred, lloss, eloss = model(feat_data_c, edges, edgesAttr_c)
            optimizer.zero_grad()
        ...
        if epoch % 10 == 1:
            ...
            print ('\n=train_eval=\n', epoch)
            st = time.time()
            evaluate(adj_lists_t, feat_data_t, labels_t, edgesAttr_t, model, device, design_names, best_acc)
            et = time.time()
            print ('!!!! overall runtime:', et - st)
            print ('\n=eval=\n', epoch)
            model.eval()
            evaluate(adj_lists_t, feat_data_t, labels_t, edgesAttr_t, model, device, design_names, best_acc)
            model.train()
...
def evaluate(adj_lists_t, feat_data_t, labels_t, edgesAttr_t, model, device, design_names, best_acc):
    test_r, test_roc = [], []
    pred_l, label_l = [], []
    for i in range(len(adj_lists_t)):
        st = time.time()
        edges, feat_data_c, labels_c, edgesAttr_c = getTensor(adj_lists_t[i], feat_data_t[i], 
                                                              labels_t[i], edgesAttr_t[i])

        pred, _, _ = model(feat_data_c, edges, edgesAttr_c)
        ...
        print ('roc_test', design_names[i], round(roc_auc_score(y_test_90, test_output)*100, 1))
